...
/lixw-cover-image path/to/article.md

# Quick mode: skip confirmation
/lixw-cover-image article.md --quick

# Specify dimensions
/lixw-cover-image article.md --type conceptual --palette warm --rendering flat-vector

# Style presets (shorthand for palette + rendering)
/lixw-cover-image article.md --style blueprint

# With reference images
/lixw-cover-image article.md --ref style-ref.png

# Direct content input
/lixw-cover-image --palette mono --aspect 1:1 --quick
[paste content]
```
...
test -f .lixw-skills/lixw-cover-image/EXTEND.md && echo "project"
test -f "${XDG_CONFIG_HOME:-$HOME/.config}/lixw-skills/lixw-cover-image/EXTEND.md" && echo "xdg"
test -f "$HOME/.lixw-skills/lixw-cover-image/EXTEND.md" && echo "user"
```

```powershell
# PowerShell (Windows)
if (Test-Path .lixw-skills/lixw-cover-image/EXTEND.md) { "project" }
$xdg = if ($env:XDG_CONFIG_HOME) { $env:XDG_CONFIG_HOME } else { "$HOME/.config" }
if (Test-Path "$xdg/lixw-skills/lixw-cover-image/EXTEND.md") { "xdg" }
if (Test-Path "$HOME/.lixw-skills/lixw-cover-image/EXTEND.md") { "user" }
```
