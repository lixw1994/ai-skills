...
输出结构依据 `references/template.md`。禁止参考 `{ProjectRootDir}/Notes/` 中已有论文文件的章节结构——旧文件可能使用过期模板。

### 文件规范

- 日期：`date +%Y-%m-%d`
- 文件名：`YYYY-MM-DD-{简短标题}.md`
- 输出目录：`{ProjectRootDir}/Notes/papers/`
...
如果论文有一张承载全文核心思路的总览图（overview / architecture diagram，通常是 Figure 1），提取并保存到 `{ProjectRootDir}/Notes/papers/images/`，文件名 `YYYY-MM-DD-{简短标题}-overview.png`。
...
获取当前日期，读 `references/template.md`，写入 `{ProjectRootDir}/Notes/papers/`。
