...
**Execution choice**:
- If multiple illustrations already have saved prompt files and the task is now plain generation, prefer `lixw-imagine` batch mode (`build-batch.ts` -> `main.ts --batchfile`)
...
|------------------------|--------|
| Yes (e.g., lixw-imagine with Google) | Pass reference images via `--ref` |
